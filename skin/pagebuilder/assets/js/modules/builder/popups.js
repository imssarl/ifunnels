...
//                observer.observe(t, config);
        ...
        createPopupsSidebar: function (catName, thePopups) {
            ...
            for( x = 0; x < thePopups.length; x++ ) {
				var cnmPath=appUI.cnmPath;
				if( thePopups[x].blocks_url.indexOf('getframe/?id') !== -1 ){
					cnmPath=appUI.siteUrl;
				}

                newItem = $('<li><img data-original-src="' + appUI.cnmPath + thePopups[x].blocks_thumb + '" data-srcc="' + cnmPath + thePopups[x].blocks_url + '" data-height="' + thePopups[x].blocks_height + '"></li>');
            ...
            $('li', popups.sideSecondPopupsNav).each(function () {
                ...
                $(this).draggable({
                    helper: function() {
                        return $('<div style="height: 100px; width: 300px; background: #F9FAFA; box-shadow: 5px 5px 1px rgba(0,0,0,0.1); text-align: center; line-height: 100px; font-size: 28px; color: #16A085; z-index: 1000"><span class="fui-list"></span></div>');
                    },
                    revert: 'invalid',
                    appendTo: 'body',
