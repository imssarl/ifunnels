(function () {
	...
	var publish = {
		...
		setRoot : document.getElementById('hosting-set-root-block'),
		buttonPublishSubmit: document.getElementById('publishSubmit'),
		publishActive: 0,
		theItem: {},
		modalSiteSettings: document.getElementById('siteSettings'),
		inputDomainSettingsDirectory: document.getElementById('domain-settings-directory'),
		...
		init: function() {
			...
			$(this.setRoot).on('change', function(){
				$('#hosting-settings-dir-block').stop(true, true).fadeToggle('fast');
			});
			$('.selectpicker').selectpicker();

			$('#domain-settings-id').on('change', this.changeDomain);
			...
			$(this.inputDomainSettingsDirectory).on('keyup blur',function(){ 
		...
		changeDomain : function(){
			if($(this).find('option:selected').parent().prop('label') == 'Domains hosted with us'){
				$('#hosting-settings-url-block,#hosting-directory-helper,#hosting-settings-dir-block,#href').stop(true, true).fadeOut('fast').prop('value', null);
				$('#hosting-set-root-block, #hosting-directory-tips').stop(true, true).fadeIn('fast');
				$('#domain-settings-directory').prop('readonly', false);
			}
			if($(this).find('option:selected').parent().prop('label') == 'Domains you host externally'){
				$('#hosting-settings-url-block, #hosting-directory-helper, #hosting-settings-dir-block, #href').stop(true, true).fadeIn('fast');
				$('#hosting-set-root-block, #hosting-directory-tips').stop(true, true).fadeOut('fast').prop('value', null);
				$('#domain-settings-directory').prop('readonly', true);
			}
		},
			...
			if( publish.publishActive === 0 ) {
				...
				if( siteBuilder.site.pendingChanges === true ) {
					...
					$('#pages li:visible').each(function(){
						...
						var theRow = $('<tr><td class="text-center" style="width: 30px;"><label class="checkbox no-label"><input type="checkbox" value="'+thePage+'" data-type="page" name="pages[]" data-toggle="checkbox"></label></td><td>'+thePage+'<span class="publishing"><span class="working">Publishing... <img src="'+appUI.baseUrl+'skin/pagebuilder/img/publishLoader.gif"></span><span class="done text-primary">Published &nbsp;<span class="fui-check"></span></span></span></td></tr>');
		 ...
		 */
		ftpUpload: function() {
			if(document.getElementById('domain-settings-directory') != null){
				
				if(!$('#hosting-set-root').prop('checked')){
					if($('#domain-settings-directory').prop('value').trim() == ''){
						$(publish.publishModal).find('.modal-alerts').empty().append(`<div class="alert alert-warning"><h4>Ouch! Something went wrong:</h4>Homepage Folder is empty.</div>`);
						return false;
					}

					if(/([^A-Za-z0-9-]+)/g.test($('#domain-settings-directory').prop('value').trim())){
						let _pathFolder = $('#domain-settings-directory').prop('value').trim().toLowerCase();
						$(publish.publishModal).find('.modal-alerts')
							.empty()
							.append(`<div class="alert alert-warning"><h4>This folder name is not valid.</h4> Please input the folder name as "${_pathFolder.replace(/([^A-Za-z0-9-]+)/g, '')}" or "${_pathFolder.replace(/([^A-Za-z0-9- ]+)/g, '').replace(/\s/g, '-')}".</div>`);
						return false;
					}
				}
			...
			$.ajax({
				...
				if(ret.responseCode === 0) {
					...
					// All good
					$('#publishModal .modal-body > .alert-success').show();
					$('#publishModal .optionPane.export, #publishSubmit, #publishModal .alert.alert-info').remove();
					$('#publishCancel').html('Close');

					$('#publishModal .modal-body-content').append(
						`<div class="alert alert-info" role="alert"> 
							<strong>URL: </strong> <a href="${ret.placement}" class="alert-link">${ret.placement}</a>. 
						</div>`
					);
				}
			});
