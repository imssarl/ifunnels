...
import { setTokenOnLinks, setToken, checkToken } from '../token';
import Swal from 'sweetalert2';
import Discounts from '../includes/discounts';

export default class Steps {
	static store = {};
	// TODO Для локальной версии http://app.local/services/deliver-request.php 
	request_url = 'https://app.ifunnels.com/services/deliver-request.php';
	customerList = null;
	checkedBump = [];

	constructor(instance) {
		this.modal = instance.getElementPopup();
	...
	/** Create a new instance of class Steps
	 * @param {object} - Object with params (Membership ID, etc.)
	 * @return {Promise} - Return promise
	 */
	static async init(instance, { membershipid, redirect_url }) {
		const _this = new Steps(instance);
		const { data } = await _this.loadConfig({ membershipid });
		Store.buffer.config = data;
		_this.request_url = data.request_url;
		Store.buffer.config.redirect_url = redirect_url || data.home_page_url;
		window.config = Object.assign(window.config, {
			request_url: data.request_url,
			primary_membership: data,
			stripe: data.stripe,
		});

		_this.initSteps();
		return Promise.resolve(Store.buffer.config);
	}
	 ...
	 */
	initSteps() {
		const { free } = Store.buffer.config;
		this.active_steps = [];
	...
	async acountStep() {
		...
		if (forgot_link) {
			forgot_link.setAttribute("href", config.primary_membership.forgot_url);
		}
		...
		if (require_shipping_el) {
			const { allowed_contries } = Store.buffer.config;
			const select = form.querySelector('select[name="country"]');

			require_shipping_el.setAttribute('data-require-shipping', Store.buffer.config.require_shipping);

			if (Store.buffer.config.require_shipping) {
		...
		form.addEventListener('submit', e => {
			...
			const { id, require_shipping } = Store.buffer.config;
			const errors = [];
			...
			axios({
				...
				data: {
					action: 'account',
					uid: window.uid,
					data: Object.assign(serialize, { primary_membership: id, require_shipping, memberships: Store.buffer.config.id })
				},
				headers: { 'Content-Type': 'multipart/form-data' }
			})
				.then(response => {
					...
					if (data.hasOwnProperty('error')) {
						...
						if (data.login) {
							...
							this.btnLogin.addEventListener('click', e => {
								...
								this.auth(data.data.cid, password.value);
							});
						}
					} else {
	...
	async paymentStep() {
		...
		const { publicKey, stripeAccount } = Store.buffer.config.stripe;
		...
		const discounts = await Payment.getDiscountList(Store.buffer.config.id, account.cid);
		...
		if (Store.buffer.discounts && Store.buffer.discounts.length) {
			...
			document.addEventListener("applyDiscount", (e) => {
				if (Store.buffer.config.frequency === 0) {
		...
		if (config.primary_membership.bump && config.primary_membership.bump.length) {
			this.generateBumpBox();
		}
		...
		const cardElement = this.steps.payment.c.querySelector('#card-element');

		if (invoice_settings.default_payment_method === null) {
			this.cardElement = Payment.initCardField(cardElement, this.stripe);
		} else {
		...
		this.btnPay.addEventListener('click', async e => {
			...
			/** Disabled btn */
			Steps.toggleLoader(true);

			const result = await Payment.requestPayment(Steps, { 
				stripe: this.stripe, 
				cardElement: this.cardElement, 
				cdata: data, 
				discount: Store.buffer.apply_discount,
				order_bump: this.checkedBump
			});
			...
			if (result.hasOwnProperty('error')) {
				...
				Store.buffer['config']['trial_free'] = result.data.trial_free;
	...
	completeStep() {
		...
		const { payment, account, config } = Store.buffer;

		if (!config.free && !config.trial_free && !payment) {
		...
		axios({
			...
			.then(({ data }) => {
				...
				if (config.redirect_url) {
					...
					this.steps.complete.c
						.querySelectorAll(".redirect")
	...
	*/
	outputDescription(container, redraw_trial = false) {
		...
		let order_amount = trial ? trial_amount : total_amount;
		...
		/** Order Bump */
		if (this.checkedBump.length > 0) {
			const bumps = config.primary_membership.bump.filter(({id}) => this.checkedBump.includes(id));

			if (bumps.length > 0) {
				bumps.forEach(item => {
					order_amount = parseFloat(order_amount) + parseFloat(item.calc_amount);

					html += `
						<div class="order-summary__detail">
							<span>${item.name}</span>
							<span></span>
							<span>${Helper.getCode(item.currency)}${item.calc_amount}</span>
						</div>`
					}
				);
			}
		...
		if (Store.buffer.apply_discount) {
			...
			if (discount) {
				...
				order_amount -= discount.discount_type == '1' ? parseFloat(discount.discount_amount) : order_amount * parseFloat(discount.discount_amount) / 100;
			}
		}

		if (order_amount - Math.floor(order_amount) > 0) {
		...
		html += `
				<div class="order-summary__total-amount">
					<span>Total</span>
					<span>${Helper.getCode(currency)}${order_amount}</span>
				</div>
			</div>
		</div>`;
		...
		if (btn) {
			btn.innerHTML = `Pay &nbsp;${Helper.getCode(currency)}${order_amount}`;
		}
	...
	static toggleLoader(show) {
		...
		if (pay.length) {
			pay.forEach(btn => btn.disabled = show);
		}
	}
	...
	*/
	...
	generateBumpBox() {
		const { bump: bumpList } = config.primary_membership;
		const container = this.steps.payment.c.querySelector(".bump-box");

		bumpList.length && bumpList.map(bump => {
			const { name, description, id, currency, calc_amount } = bump;
			const bumpNode = document.createElement('div');

			bumpNode.classList.add('bump-box__item');

			bumpNode.innerHTML = `
				<div class="bump-box__item__title">
					<div class="bump-box__item__title__checkbox">
						<input type="checkbox" value="${id}" id="membership-${id}" />
					</div>

					<div class="bump-box__item__title__text">
						<label for="membership-${id}">
							<span>Yes!</span> <span>Add the ${name} to my order</span> <span>${Helper.getCode(currency)}${calc_amount}</span>
						</label>
					</div>
				</div>`;

				if (description) {
					bumpNode.innerHTML += `<div class="bump-box__item__description">${description}</div>`;
				}

			container.appendChild(bumpNode);
		});

		if (bumpList) {
			container
				.querySelectorAll('.bump-box__item input[type="checkbox"]')
				.forEach(el => el.addEventListener(
					'change',
					e => {
						if (e.currentTarget.checked) {
							this.checkedBump.push(e.currentTarget.value);
						} else {
							this.checkedBump = this.checkedBump.filter(i => i !== e.currentTarget.value);
						}

						this.outputDescription(this.steps.payment.c.querySelector('#payment-form'), window.config.primary_membership.trial);

						console.log('Checked Bump: ', this.checkedBump);
					}
				));
		}
	}
	...
	generateDisCountBox() {
		if (!document.querySelector('style.discount')) {
			import('!style-loader?{"attributes":{"class":"discount"}}!css-loader!sass-loader!../../scss/discount/index.scss');
		}

		const { discounts } = Store.buffer;
		const container = this.steps.payment.c.querySelector('.discount-box');

		container.innerHTML = `<div class="form-subtitle">${discounts.length == 1 ? 'Good news, you qualify for the following discount:' : 'Good news! We\'ve secured a few discounts for you! Pick the one you want!'}</div>`;
		discounts.forEach(({ name, discount_amount, discount_type }) => {
			container.innerHTML += `<div class="discount-item">
				<span class="discount-name">${name}</span> <span class="discount-amount">${Helper.getDicountType(discount_type)}${discount_amount}</span> <button class="discount-apply"><i class="check circle outline icon"></i> Apply Now</button>
			</div>`;
		});
	}
}