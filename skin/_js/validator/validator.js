...
Form.Validator.add('validate-one-required-names', {
	errorMsg: Form.Validator.getMsg.pass('oneRequired'),
	test: function(element, props){
		return $$('.validate')[0].getElements('input[name="'+ element.get('name') +'"]').some(function(el){
			if (['checkbox', 'radio'].contains(el.get('type'))) return el.get('checked');
			return el.get('value');
		});
	}
...
Form.Validator.add('validate-one-required-class', {
	errorMsg: Form.Validator.getMsg.pass('oneRequired'),
	test: function(element, props){
		return $$('.validate input.'+ props['class']).some(function(el){
			if( el.get('disabled') ){
				return false;
			}
...
Form.Validator.add('validate-natural-number', {
	errorMsg: function(element, props){
...
var WhValidatorInit=new Class({
	...
	initialize: function( form ){
		this.parent(form, {
			...
			showError: function(errorElement){
				errorElement.setStyle('display', 'inline');
			},
	...
	initEvent: function(){
		...
		this.addEvent('onElementFail', function( field ){
			...
			if( typeof objAccordion!=='undefined' ){
				this.stop();
				var block = parseInt( field.getParent('div.panel-collapse' ).id );
	...
	rebuild: function( field ){
		var elements=field.getParent().getChildren('.validation-advice');
		elements.each(function(element){
			element.getParent().setStyle('position','relative');
			element.addClass('input-notification error png_bg');
		});
