...
//					var span = new Element( 'span', { 'id':'id-'+uId,'html':range.text } );
	...
	initEvent: function(){ 
		...
		$$(this.options.conteiner).each(function(element){ 
			element.addEvent('mouseup',function(e) {	
				...
				if ( !this.objSelection.text ) { // If not selected text
					return;
				}
	...
	getDefaultVariations: function(){
		...
		var request = new Request.MyJSON({
			...
			onSuccess: function( res ){ 
				...
				if( !res || res['arrVariations'].length == 0 ){ $(objThis.options.systemSynonymsBlock).setStyle('display','none');	return false; }
	...
	setColor: function(){
		if( this.objSelection.flg_color ){
			return false;
		}
	...
	getRandomColor: function(){
		var r = Number.random(40, 220);
		var g = Number.random(40, 220);
        var b = Number.random(40, 220);
	...
	setUserVariationsData: function(){
		if( !$(this.options.conteinerUserVariations).get('value') ) {
			return false;
		}
		var variations = $( this.options.conteinerUserVariations ).get('value').replace(/\r/g,'').replace(/\n/g,'|');
