...
window.addEventListener("load", function(){
	$$('.go_to_page').addEvent('click',function(e){
		e&&e.stop();
		if( $($( e.target.previousElementSibling ).children[0]).get('value') != '' )
			window.location=$( e.target ).get('data-href')+$($( e.target.previousElementSibling ).children[0]).get('value');
	});
});
