...
/**
 * Linkshare контент функционал
 */

class Project_Content_Adapter_Linkshare implements Project_Content_Interface {
	...
	public function getList( &$mixRes ) {
		...
		$httpsTail=http_build_query($_tmp);
		$client=new Zend_Http_Client(
			"http://productsearch.linksynergy.com/productsearch?".$httpsTail.(($this->_paging['curpage']>1)?"&pagenumber=".$this->_paging['curpage']:'')."&MaxResults=".$this->_limit,
			array(
		...
		$this->_paging['recall'] = (int)$xml->TotalMatches;
		foreach ($xml->item as $item) {
			...
			$mixRes[$index]=array (
				...
				'shortDescription'	=> (string)$shortDescription[0],
				'image'			=> (string)$img[0],
	...
	public function prepareBody( &$mixRes ){
		foreach( $mixRes as &$_item ){
			...
			$path=Zend_Registry::get( 'config' )->path->relative->source.'site1_publisher'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR.'linkshare'.DIRECTORY_SEPARATOR;
