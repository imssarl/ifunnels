<?php
class Project_Sites_Adapter_Blogfusion_Import {
	...
	public function start() {
		...
		// дёргаем
		if ( !Core_Curl::getResult( $_strRes, $this->_data->filtered['url'].'cnm-import.php' ) ) {
			return Core_Data_Errors::getInstance()->setError( 'no respond '.$this->_data->filtered['url'].'cnm-import.php' );
		}
	...
	private function generateImporter() {
		...
		$_str.=self::getCodeXml();
		// временная дира
		$this->_dir='Project_Sites_Adapter_Blogfusion_Import@generateImporter';
	...
	private function putDatas() {
		...
		if ( !empty( $this->_result['options'] ) ) {
			...
			if ( $theme->onlyOne()->withTitle( $this->_result['options']['current_theme'] )->getList( $arrTheme )->checkEmpty() ) {
			...
			if ( !empty( $this->_result['options']['active_plugins'] ) ) {
				...
				if ( $plugin->onlyIds()->withFilenames( $arrName )->getList( $arrPluginIds )->checkEmpty() ){
