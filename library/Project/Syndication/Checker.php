	...
	public function run() {
		...
		$_arrUrls=Core_Sql::getAssoc( '
			SELECT c.*, (
				...
				WHERE  user_id IN ('.Core_Acs::haveRightAccess(array('services_@_syndication_post_content')).') AND id=(SELECT site_id FROM '.Project_Syndication::$tables['content2site'].' WHERE id=c.shedule_id)
			) site_owner
			FROM cs_checker c
