...
class Project_Wizard_Adapter_Authority implements Project_Wizard_Adapter_Interface{
	...
	private function prepare(){
		...
		if( !$_content->setSettings( $this->_settings['settings'] )->getList( $_temp )->checkEmpty() ){
			return Core_Data_Errors::getInstance()->setError('Can\'t find content');
		}
		self::$_jsonKeys=json_encode(array_keys( $_temp ));
		self::$_contentCount=count( $_temp );
		return true;
	}
	...
	public function createAllAtOnce(){
		...
		if( !$_publisching->setData( array(
			...
			'flg_rewriting'=>0,
			'selectdepth'=>0,
			'title'=>'Amazon Wizard: Content for site '. $this->_site['main_keyword'],
			'category_id'=>$this->_data->filtered['category_id'],
			'flg_posting'=>3,
