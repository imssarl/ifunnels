...
class Core_Media_Ftp {
	...
	public function setMode( $mode=null ) {
		if ( !in_array( $mode, array( self::TMODE_ASCII, self::TMODE_BINARY ) ) ) {
			return $this->_error->setError( 'transfermode has either to be Core_Media_Ftp::TMODE_ASCII or Core_Media_Ftp::TMODE_BINARY' );
		}
		$this->_mode=$mode;
		return true;
	}
	 ...
	 * @return void
	 */
	private function setPassive() {
		@ftp_pasv( $this->ftp, $this->_passv );
	}
	...
	public function makeConnectToRootDir() {
		if ( empty( $this->_root ) ) {
			return $this->_error->setError( 'empty rootdir variable' );
		}
		if ( !$this->makeConnect() ) {
			return false;
		}
	...
	public function makeConnect() {
		...
		if ( empty( $this->_host ) ) {
			return $this->_error->setError( 'empty hostname variable' );
		}
		$this->ftp=@ftp_connect( $this->_host );
		if ( $this->ftp===false ) {
			return $this->_error->setError( 'wrong ftp_connect( '.$this->_host.' )' );
		}
	...
	private function makeLogin() {
		...
		if ( empty( $this->_user )||empty( $this->_passw ) ) {
			return $this->_error->setError( 'empty authorization variables' );
		}
		if ( !@ftp_login( $this->ftp, $this->_user, $this->_passw ) ) {
			$this->closeConnection();
			return $this->_error->setError( 'wrong ftp_login('.$this->_user.', '.$this->_passw.')' );
		}
		$this->setPassive();
		return true;
	}
	...
	public function fileUpload( $_strRemote='', $_strLocal='' ) {
		if ( empty( $_strRemote )||empty( $_strLocal ) ) {
			return $this->_error->setError( 'empty $_strRemote or $_strLocal' );
		}
		if ( !file_exists( $_strLocal )||!is_file( $_strLocal ) ) {
			return $this->_error->setError( $_strLocal.' not a file or not exists' );
		}
		if ( !@ftp_put( $this->ftp, $_strRemote, $_strLocal, $this->_mode ) ) {
			return $this->_error->setError( 'ftp_put(): '.$_strLocal.' >> '.$_strRemote );
		}
	...
	public function fileDownload( $remoteFile, $localFile, $overwrite=false ) {
		if ( @file_exists( $localFile )&&!$overwrite ) {
			return $this->_error->setError( 'local file '.$localFile.' exists and may not be overwriten.' );
		}
		if ( @file_exists( $localFile )&&!@is_writeable( $localFile )&&$overwrite ) {
			return $this->_error->setError( 'local file '.$localFile.' is not writeable. Can not overwrite.' );
		}
		$res=@ftp_get( $this->ftp, $localFile, $remoteFile, $this->_mode );
		if (!$res) {
			return $this->_error->setError( 'remote file '.$remoteFile.' could not be downloaded to $localFile.' );
		}
		return true;
	}
	/**
	...
	public function chmod( $_mix=array(), $_flg=self::CHMOD_FILE ) {
		...
		if ( is_array( $_mix ) ) {
			...
			if ( !@ftp_chmod( $this->ftp, intval( $_str, 8 ), $_mix ) ) {
				return $this->_error->setError( 'wrong ftp_chmod '.$_mix.' to '.$_str );
			}
		}
		return true;
	}
	...
	public function makeDir( $_strDir='' ) {
		if ( empty( $_strDir ) ) {
			return $this->_error->setError( '$_strDir is empty' );
		}
		if ( !@ftp_mkdir( $this->ftp, $_strDir ) ) {
			return $this->_error->setError( 'wrong ftp_mkdir '.$_strDir );
		}
	...
	private function changeOrMakeDir( $_strDest='' ) {
		if ( empty( $_strDest ) ) {
			return $this->_error->setError( 'empty $_strDest' );
		}
	...
	public function pwd( &$strRes ) {
		...
		if ( $strRes===false ) {
			return $this->_error->setError( 'Could not determine the actual path.' );
		}
		return true;
	}
	...
	public function rmDir( $_strRemote='' ) {
		if ( empty( $_strRemote ) ) {
			return false;
		}
		$_flgRemoveDir = @ftp_rmdir( $this->ftp, $_strRemote );
		if( $_flgRemoveDir === false ){
			if( ftp_rawlist($this->ftp, $_strRemote, true) === false ){
				return true; // такой папки уже нет
			}
			$_flgRemoveDir=$this->recursiveDelete( $this->ftp, $_strRemote );
		}
		return $_flgRemoveDir;
	}

	function recursiveDelete($ftp_stream, $directory){
		...
		foreach ($folders as $folder) {
			$this->recursiveDelete($ftp_stream, $folder);
		}
	...
	public function cd( $_strDir='' ) {
		if ( !@ftp_chdir( $this->ftp, $_strDir ) ) {
			return $this->_error->setError( 'Directory change (ftp_chdir( $this->ftp, '.$_strDir.' )) failed' );
		}
		return true;
	}
	...
	public function ls( &$arrRes, $_mode=self::LS_DIRS_FILES ) {
		if ( empty( $this->_dirForLs ) ) {
			if ( !$this->pwd( $_strDir ) ) {
				return $this->_error->setError( 'Could not retrieve current directory' );
			}
			$this->_dirForLs=$_strDir;
		}
		switch( $_mode ) {
	...
	private function getRawList( &$arrList ) {
		...
		if ( $this->isPassive() ) {
			$this->setTransfer(false)->setPassive();
			return $this->getRawList( $arrList );
		}
		return $this->_error->setError( 'Could not get raw directory listing' );
	}
	...
	private function determineOSMatch( &$dir_list ) {
		...
		return $this->_error->setError( 'The list style of your server seems not to be supported' );
	}
	...
	private function parseDate($date) {
		...
		$res=strtotime($date);
		if (!$res) {
			return $this->_error->setError( 'Dateconversion failed' );
		}
		return $res;
	}
	...
	public function getErrors( &$arrErrors ) {
		$arrErrors=$this->_error->getErrorsFlow();
		return $this;
	}

	/**
	 * например сообщение о удачно проведённой операции
	 * можно ставить например return $this->_error->setError() вместо return false
	 * DEPERCATED!!! 15.05.2012
	 *
	 * @return bool
	 */
	public function setError( $_strError='' ) {
		$this->_error->setError( $_strError );
		return false;
	}
}
?>