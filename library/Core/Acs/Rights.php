 ...
 */
class Core_Acs_Rights extends Core_Data_Storage {

	public static $tableLink='u_rights2group';

	protected $_table='u_rights';
	...
	public function getRightWithModule( &$arrRes ) {
		$_arrS=Core_Sql::getAssoc( '
			(
				SELECT r.*, 3 flg_tree, 0 mid, "Added manually" mtitle
				FROM u_rights r
				WHERE r.sys_name NOT LIKE "%_@_%"
			) UNION (
				SELECT r.*, a.flg_tree, m.id mid, m.title mtitle
				FROM sys_module m
				INNER JOIN sys_action a ON a.module_id=m.id
				LEFT JOIN u_rights r ON r.sys_name=CONCAT(m.name,"_@_",a.action)
			)
			ORDER BY flg_tree, mid, mtitle
		' );
		if ( empty( $_arrS ) ) {
			return false;
		}
		foreach( $_arrS as $v ) {
			$arrRes[$v['flg_tree']][$v['mid']][]=$v;
		}
		return !empty( $arrRes );
	}
}
