<?php
class Core_Sql_Assembly {

	public function setPaging() {}

	public function getUnionSql() {}

	private function getUnionFull() {}

	private function getUnionCounter() {}
...
class Core_Sql_Qcrawler extends Core_Services {
	...
	public function gen_union_counter() {
		...
		$_arrU=array();
		foreach( $this->q_union as $v ) {
			$v->get_result_counter( $_arrU[] );
		}
		$this->q_result_counter='('.join( ') UNION ALL (', $_arrU ).')';
		return true;
	}
	...
	public function get_union_sql( &$strSql, &$arrPg, $_arrSet=array() ) {
		...
		if ( !empty( $this->page_setting ) ) {
			if ( empty( $this->page_setting['rowtotal'] ) ) {
				...
				$_arrC=Core_Sql::getField( $this->q_result_counter );
				$this->page_setting['rowtotal']=array_sum( $_arrC );
			}
			$this->getPaged( $strSql, $arrPg, $this->q_result_full, $this->page_setting );
		}
		return $strSql;
	}
	...
	public function set_union_select( mysql_qcrawler $obj ) {
		$this->q_union[]=$obj;
	}
...
*/
	...
	function m_paged_url() {
		...
		$_strHref.=Core_Module_Router::$uriFull.'?';
