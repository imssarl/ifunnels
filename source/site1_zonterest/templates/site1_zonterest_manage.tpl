...
{if !$arrPrm.flg_tpl}
	{if $msg}
		{if $msg=='deleted'}
			{include file="../../message.tpl" type='info' message="Site deleted"}
		{/if}
		{if $msg=='file_format_error'}
			{include file="../../message.tpl" type='error' message="Please upload your image in .jpeg, .png, or .gif format."}
		{/if}
		{if $msg=='image_updated'}
			{include file="../../message.tpl" type='info' message="Image uploaded successfully"}
		{/if}
		{if $msg=='changed'}
			{include file="../../message.tpl" type='info' message="Site changed successfully"}
		{/if}
		{if $msg=='error'}
			{include file="../../message.tpl" type='error' message="Site edited error"}
		{/if}
	{/if}
	{if $error}
	...
	{/if}
{/if}
{assign var="unlimitedString" value=""}
{if $zCounter!="unlimited"}
{$unlimitedString=' If you would like to create more sites, <script async src="//thrivecart.com/embed/v1/thrivecart.js"></script><a data-thrivecart-account="ims" data-thrivecart-product="11" class="thrivecart-button"> click here to Upgrade</a>.'}
{/if}
{include file="../../message.tpl" type='success' message="To get started, make sure to follow these steps and instructions: <a href='{url name='site1_zonterest' action='getstarted'}'>Getting Started Guide</a>"}
{if $zCounter!=0}
{include file="../../message.tpl" type='attention' message=sprintf("Now you can create and manage %s AzonFunnels site%s in total.%s", $zCounter, ( ($zCounter!=1 || $zCounter=="unlimited")?"s":"" ), $unlimitedString )}
{elseif $zCounter!="unlimited" }
{include file="../../message.tpl" type='attention' message=sprintf("Now you can't create and manage AzonFunnels sites.%s", $unlimitedString)}
{/if}
...
<table class="table  table-striped">
	...
	<tr{if $k%2=='0'} class="matros"{/if}>
		...
		<td><img{if $preview} class="screenshot" rel="<div style='border:2px solid #000000;'><img src='{$preview}{assign var=preview value=''}' /></div>"{/if} src="{img src=$image w=95 h=60}{assign var=image value=''}" /><br />{$arrTemplates[$v.template_id].temp_name}</td>
		<td>{$v.main_keyword}</td>
		{/if}
		<td><a href="{$v.url}" target="_blank">{$v.url}</a></td>
		<td width="{if $arrPrm.flg_tpl}360{else}150{/if}">{if $v.category}{$v.category}{else}<a class="mb select-category"  href="#mb"  title="Select category" rev="{$v.id}">Select category</a>{/if}</td>
		{if !$arrPrm.flg_tpl}
		<td class="option">
			<a {is_acs_write} href="{url name='site1_zonterest' action='edit'}?id={$v.id}" id="{$v.id}" rel="{$v.category_id}" title="Edit"><i class="ion-edit" style="font-size: 18px; vertical-align: bottom; color: #FFCC29; margin: 0 5px;"></i></a>
			<a {is_acs_write} href="#upload_{$v.id}" rel="{$v.id}" class="popup_upload" title="Upload Site Screenshot"><i class="ion-image" style="font-size: 18px; vertical-align: bottom; color: #FFCC29; margin: 0 5px;"></i></a>
			<div style="display:none;">
				<div id="upload_{$v.id}">
					<div class="card-box">
						<form action="" class="wh row" method="post" enctype="multipart/form-data">
							<div class="col-md-8">
								<div class="m-t-5">
									<input type="file" name="file" />
								</div>
								<input type="hidden" name="updFile[id]" value="{$v.id}" />
							</div>
							...
							<div class="col-md-12 m-t-10">	
								<div class="alert alert-warning">
									Please upload your image in <strong>.jpeg</strong>, <strong>.png</strong>, or <strong>.gif</strong> format.
								</div>
							</div>
						</form>
					</div>
			...
			<a {is_acs_write} href="{url name='site1_zonterest' action='manage'}?del={$v.id}" class="delete" title="Delete"><i class="ion-trash-a" style="font-size: 20px; vertical-align: bottom; color: #AC1111; margin: 0 5px;"></i></a>
			{if !Core_Acs::haveAccess( array('Zonterest PRO') )}
			{*if !Core_Acs::haveAccess( array( 'DFY' ) )}
			<a {is_acs_write} href="{url name='site1_blogfusion' action='create'}?ncsb={$v.id}" title="Install Wordpress"><i class="ion-social-wordpress" style="font-size: 20px; vertical-align: bottom; margin: 0 5px;"></i></a>
...
<script type="text/javascript">
	multibox=new CeraBox( $$('.popup_upload'), {
		group: false,
		displayTitle: true,
		titleFormat: '{title}',
		width:'20%',
...
<div id="mb">
	<form action="" class="wh" style="padding:10px" method="POST" >
		...
		<select class="second medium-input" name="arrNewCat[category_id]"><option value="">- select -</option></select><br /><br />
		<input type="submit" value="Change" class="button" {is_acs_write}>
		<br /><br />
	</form>
</div>
</div>
