...
<table class="table  table-striped">
	...
	<tr id="row{$id}"{if $id%2=='0'} class="matros"{/if}>
		...
		<td align="center">{count($split.arrCom)}</td>
		<td align="center">{$split.added|date_format:$config->date_time->dt_full_format}</td>
		<td align="center">{if $split.flg_duration=='1'}{$split.duration} days{elseif $split.flg_duration=='2'}{$split.duration} views{else}Not Restricted{/if}</td>
		<td align="center">{if $split.flg_closed!='0'}Completed{else}Running{/if}</td>
		<td>
			<a {is_acs_write} href="{url name='site1_squeeze' action='create_split'}?id={$split.id}" title="Edit">
				<i class="ion-edit" style="font-size: 18px; vertical-align: bottom; color: #FFCC29; margin: 0 5px;"></i>
			</a>
			<a {is_acs_write} href="{url name='site1_squeeze' action='manage_split'}?split_del_id={$split.id}" title="Delete" class="alert1" alt="Do you want to delete this Split test?">
				<i class="ion-trash-a" style="font-size: 20px; vertical-align: bottom; color: #AC1111; margin: 0 5px;"></i>
			</a>
			<a {is_acs_write} href="{url name='site1_squeeze' action='manage_split'}?split_duplicate_id={$split.id}" title="Duplicate" class="alert1" alt="Do you want to duplicate this Split test?">
				<i class="ion-ios7-copy-outline" style="font-size: 20px; vertical-align: bottom; color: #99AABB; margin: 0 5px;"></i>
			</a>
			{if ( empty( $split.flg_closed ) && empty($split.flg_pause))}
			...
			{elseif $split.flg_pause == '1'}
			<a {is_acs_write} href="#" class="request" rel="resume" key="{$split.id}" title="Set to resume">
				<i class="ion-ios7-play" style="font-size: 20px; vertical-align: bottom; margin: 0 5px;"></i>
			{else}
			<a {is_acs_write} href="" class="open_div" id_num="{$split.id}" style="cursor:pointer" title="Split test is over and a winning campaign is now running.Click here to view.">
				<i class="ion-ios7-play" style="font-size: 20px; vertical-align: bottom; margin: 0 5px;"></i>
			{/if}
			</a>
			<a {is_acs_write} style="cursor:pointer" rel="" href="{url name='site1_squeeze' action='getlink'}?type=splittest&id={$split.id}" title="Get code" class="popup" title="View Code">
				<i class="ion-code" style="font-size: 20px; vertical-align: bottom; color: #4E0D7A; margin: 0 5px;"></i>
			</a>
			<a class="open_div" id_num="{$split.id}" title="Click Here To Expand" style="cursor:pointer">
	...
	<tr>
		...
		<div id="split{$split.id}" style="display:none;">
			<table style="width:98%;" align="center" class="table table-striped">
			<tr>
				<th>Page Name</th>
				<th class="text-center"># of impressions</th>
				<th class="text-center"># of clicks/optins</th>
				<th class="text-center">Goal 1</th>
				<th class="text-center">Goal 2</th>
				<th class="text-center">Goal 3</th>

				<th class="text-center">Effectiveness Rate</th>
			...
			<tr{if $pid%2=='0'} class="matros"{/if}>
				...
				<td align="center" valign="top">{$company.shown}</td>
				<td align="center" valign="top">{$company.clicks}</td>
				<td align="center" valign="top">{$company.stat.goal1}</td>
				<td align="center" valign="top">{$company.stat.goal2}</td>
				<td align="center" valign="top">{$company.stat.goal3}</td>
				<td align="center" valign="top">{$company.crt}&nbsp;%</td>
				<td align="center">
					...
					{else}
					<a {is_acs_write} href="#" class="request"  rel="winner" key="{$split.id}" imag="br" cid="{$company.id}">
					...
					<a href="{url name='site1_squeeze' action='conversion_pixel'}?id={$company.id}" class="popup" title="Get Statistics"><i class="ion-stats-bars" style="color: #5fbeaa; font-size: 20px; vertical-align: bottom; margin: 0 5px;"></i></a>
				</td>
...
var manage_splitrClass = new Class({
			...
			el.addEvent('click', function(e){
				...
				if ( el.get('rel') != 'stoped'){
					if ( (el.get('rel') =="winner" || el.get('rel') =="winner_q") && confirm("Do you want to make this Campaign as a winner?") ){
						this.requestRel(el);
					} else if ( el.get('rel') =="pause" && confirm("Are you sure you want to pause split test?") ){
						this.requestRel(el);
					} else if ( el.get('rel') == "resume" && confirm("Are you sure you want to restore split test?")){
						this.requestRel(el);
					}
				}
			}.bind(this));
	...
	requestRel: function (el){
		this_class = this;
		var r=new Request({
			url: '{/literal}{url name="site1_squeeze" action="request"}{literal}',
			onRequest: function(){
				el.getChildren('i').set({'class':'fa fa-spinner fa-spin','style':'font-size: 17px; vertical-align: bottom;','title':'Work'});
			},
	...
	trueRequest: function (el){
		if ( el.get('rel') == 'pause' ){
			el.set({'rel':'resume'})
			.getChildren('i')
			.set({'class':'ion-ios7-play','style':'font-size: 20px; vertical-align: bottom; margin: 0 5px;','title':'Split test is over and a winning campaign is now running.Click here to view.'});
		} else if (el.get('rel') == 'resume' ){
			el.getChildren('i').set({'class':'ion-ios7-pause','style':'font-size: 20px; vertical-align: bottom; margin: 0 5px;','title':'Set to pause'});
			el.set({'rel':'pause'});
