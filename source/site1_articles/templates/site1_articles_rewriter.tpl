...
<fieldset id="button-block">
	<p>
		<input type="button" class="clipboard-click clipboard-id-1 button" value="Copy to Clipboard" />
		{if !$isLast}<input type="submit" id="rewrite" value="Rewrite It Again" class="button" />{/if}
		<input type="button"  class="button" id="save-one" value="Save to Content Wizard" />
		<input type="button"  class="button" id="save-all" value="Save All Variations" />
	</p>
</fieldset>
<fieldset id="all-variations" style="display:none;">
	<p>
		<label>Number of times to rewrite</label><input type="text" id="max" maxlength="2" value="15" class="text-input medium-input" />
		<small>Max:15</small>
	</p>
	<p>
		<input type="radio" name="type-save" class="type-save" value="3" checked='1' />&nbsp;Export
		<input type="radio" name="type-save" class="type-save" value="2" />&nbsp;Save to content wizard
	</p>
	<p>
		<input type="button" class="button" {is_acs_write} value="Back" id="all-back">&nbsp;<input type="submit" value="Save" id="save" class="button" {is_acs_write} />
	</p>
</fieldset>
...
window.addEvent('domready',function(){ 
	...
	$('rewrite').addEvent('click', function(e){
		e.stop();
		$('type-action').set('value',0);
		$('form-rewrite').submit();
	});
...
<form class="wh" method="POST">
	...
	<fieldset>
		...
		<p id="select-article-block" style="display:block;">
			{module name='site1_articles' action='multiboxplace' place='select_one' type='multiple' input='radio'}
		</p>
		...
		<div id="settings" style="display:none;">
			...
			<p>
				...
				<input type="button" class="button" value="Apply" id="apply"/>
				<input type="button" class="button" value="Apply to all" id="apply-all"/>
			</p>
		</div>
		<div id="rewrite" style="display:{if empty($arr)}none{else}block{/if};">
			<p>
				<input type="submit" class="button" value="Rewrite" id="submit" >
				<input type="button" class="button" value="Cancel" id="cancel-all" >
...
var articleList = new Class({
	...
	cunstructHTML: function(){
		...
		}).post({ get_article:true,id:this.ids[this.ids.length-1].id });
	},
	set: function(){
