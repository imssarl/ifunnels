	                  			...
	                  			<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: separate; mso-table-lspace: 0pt; mso-table-rspace: 0pt; width: 100%;" width="100%">
	                    			<tr>
	                      				<td style="font-family: sans-serif; font-size: 14px; vertical-align: top;" valign="top">
	                      					<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">Dear {if empty($buyer_name) && empty({$buyer_surname})}User{else}{$buyer_name} {$buyer_surname}{/if},</p>
					                      	<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">Thank you for joining {$moduls}.</p>
											<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
												Here are your login details to access the service: <a href="{Zend_Registry::get( 'config' )->domain->url}">{Zend_Registry::get( 'config' )->domain->url}</a><br/>
												Login: {$email}<br />
												{if !empty($password)}Password: {$password}<br/>{/if}
											</p>
											<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
												You might as well retrieve your password at any time here:<br/>
												<a href="http://{Core_Module_Router::$domain}{url name='site1_accounts' action='change'}?code={$code_forgot}">http://{Core_Module_Router::$domain}{url name='site1_accounts' action='change'}?code={$code_forgot}</a>
											</p>
											<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">
												In order to access the training and support, please login using your credentials above and then use the online tutorials and help widgets.
											</p>
											<br/>
											<p style="font-family: sans-serif; font-size: 14px; font-weight: normal; margin: 0; margin-bottom: 15px;">iFunnels Team<br/>JP Schoeffel<br/>
											</p>
	                      				</td>
	                    			</tr>
	                  			</table>
