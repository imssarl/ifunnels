...
<table class="table  table-striped">
	...
	<tr{if $k%2=='0'} class="matros"{/if}>
		...
		{if !$arrPrm.flg_tpl}
		<td class="option">
			<a {is_acs_write} href="{url name='site1_ncsb' action='edit'}?id={$v.id}" id="{$v.id}" rel="{$v.category_id}" title="Edit"><i class="ion-edit" style="font-size: 18px; vertical-align: bottom; color: #FFCC29; margin: 0 5px;"></i></a>
			<a {is_acs_write} href="{url name='site1_ncsb' action='manage'}?del={$v.id}" class="delete" title="Delete"><i class="ion-trash-a" style="font-size: 20px; vertical-align: bottom; color: #AC1111; margin: 0 5px;"></i></a>
			{if !Core_Acs::haveAccess( array('Zonterest PRO') )}
			<a {is_acs_write} href="{url name='site1_ncsb' action='edit'}?id={$v.id}&template=change" title="Change template"><img src="/skin/i/frontends/design/buttons/template.png" /></a>
