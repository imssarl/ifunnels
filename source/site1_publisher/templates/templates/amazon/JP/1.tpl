<div id="item_{$ASIN}">
	...
	<div class="amz_box">
		...
		<div class="amz_right_box">
			...
			<b>参考価格:</b>&nbsp;<span class="amz_list_price">&nbsp;{$oldPrice}</span><br/>
	...
	<div class="amz_content_box">
		<span class="amz_details_header">製品の説明</span><br/>
		<p>{$body}</p><br/>
		{if !empty($feature)}<span class="amz_details_header">細部</span><br/><p>{$feature}</p><br/>{/if}
		{if !empty({$relatedItems})}<br/><span class="amz_details_header">関連アイテム</span><br/><p>{$relatedItems}</p>{/if}<br/>
		<center>
			<a target="_blank" href="{$link}" class="amz_more_info">
				<img src="images/more_info.png"/>
			</a>
