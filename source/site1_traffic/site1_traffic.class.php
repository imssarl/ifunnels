 ...
 * @copyright Copyright (c) 2009-2015, web2innovation
 * @author Rodion Konnov <kindzadza@mail.ru>
 * @author Viacheslav Slepov <shadowdwarf@mail.ru>
 * @date 23.04.2015
 * @version 1.0
 */
...
class site1_traffic extends Core_Module {
	...
	public function set_cfg(){
		$this->inst_script=array(
			...
			'actions'=>array(
				array( 'action'=>'create', 'title'=>'Traffic Locator create', 'flg_tree'=>1 ), // old action
				...
				array( 'action'=>'edit_url', 'title'=>'URL' ),
	...
	public function campaign(){
		...
		if( isset( $_POST['submit'] ) ){
			if( !$_campaigns->setEntered( $_POST['arrData'] )->set() ){
				...
				$this->objStore->set( $this->out+array( 'error'=>implode( '<br/>', $this->out['arrErrors']['errFlow'] ) ) );
				$this->location();
			}else{
				$_campaigns->getEntered( $this->out['arrCampaign'] );
				$this->objStore->toAction( 'manager' )->set( array( 'msg'=>'success', 'campaign'=>$this->out['arrCampaign'] ) );
		...
		$this->out['trafficCredits']=Core_Users::$info['statistic']['traffic_credits'];
	...
	public function manager(){
		$this->objStore->getAndClear( $this->out );
		if( isset( $this->out['campaign'] ) ){
			$category=new Core_Category( 'Trafic Exchanger' );
			$category->getTree( $arrCategories );
			$_arrCategories=array();
			foreach( $arrCategories as $_e ){
				$_arrCategories[$_e['id']]=$_e['title'];
			}
			Core_Users::$info['send2Intercome']=array(
				'json'=>json_encode( array(
					'xchange_url'=>$this->out['campaign']['url'], 
					'xchange_category'=>$_arrCategories[$this->out['campaign']['category_id']],
					'xchange_credits'=>(int)$this->out['campaign']['credits']
				) ),
				'action'=>'Traffic-Exchange-Campaign-Created'
			);
		}
		$this->showCredits();
		$_campaigns=new Project_Traffic();
		if( isset( $_GET['end'] ) && !empty( $_GET['end'] ) ){
			if( !$_campaigns->withIds( $_GET['end'] )->end() ){
				$this->objStore->set(array( 'error'=>'end' ));
			}else{
	...
	public function promote(){
		...
		foreach( $this->out['arrList'] as &$_item ){
			if( !empty( $_item['screenshot_url'] )
				&& !is_file( Zend_Registry::get('config')->path->absolute->user_files.'squeeze'.DIRECTORY_SEPARATOR.'backgrounds'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).'.jpg' ) 
				&& !is_file( Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).'.jpg' )
			){
				if( $_limit == 0 ){
					continue;
				}else{
				...
				Project_Squeeze::getImageFromLink( $_item['screenshot_url'] );
				rename(
					Zend_Registry::get('config')->path->absolute->user_files.'squeeze'.DIRECTORY_SEPARATOR.'backgrounds'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg",
					Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg"
				);
				$_item['image']=Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg";
			}elseif( is_file( Zend_Registry::get('config')->path->absolute->user_files.'squeeze'.DIRECTORY_SEPARATOR.'backgrounds'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).'.jpg' ) ){
				rename(
					Zend_Registry::get('config')->path->absolute->user_files.'squeeze'.DIRECTORY_SEPARATOR.'backgrounds'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg",
					Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg"
				);
				$_item['image']=Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg";
			}elseif( is_file( Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).'.jpg' ) ){
				$_item['image']=Zend_Registry::get('config')->path->absolute->user_files.'traffic'.DIRECTORY_SEPARATOR.'screenshots'.DIRECTORY_SEPARATOR.md5( $_item['screenshot_url'] ).".jpg";
			}
		}
		$category=new Core_Category( 'Trafic Exchanger' );
		...
		if( isset( $_GET['browse'] ) ){
			if( empty( $_GET['browse'] ) ){
				...
				if( !empty( $arrCampaigns[$_item] ) ){
					$_url=$arrCampaigns[$_item]['url'];
				}else{
					$_url=$_campaigns->getDefaultPage();
				}
			}else{
				$_url=$_GET['browse'];
			}
			header( 'Location: '.$_url.'&referer' );
			exit;
		}
	}
	...
	public function edit_url(){
		...
		if( isset( $_POST['url'] ) && !empty( $_POST['url'] ) ){
			$_campaigns->setDefaultPage( $_POST['url'] );
			unset( $_POST );
		}
		$this->out['url']=$_campaigns->getDefaultPage();
	}
