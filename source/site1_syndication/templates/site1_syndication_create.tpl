...
{foreach from=$arrErr item=i}
	{include file='../../message.tpl' type='info' messages=$i}
{/foreach}
{/if}
<form action="" class="wh validate" id="create-form" method="post">
	{if $arrPrj.id}
	...
	<fieldset {if $arrPrj.flg_status==$arrStatus.progress} style="display:none;" {/if}>
		...
		<p>
			<input type="checkbox" id="video-content" {if  $arrPrj.flg_status>=$arrStatus.pending}disabled='1'{/if} /> Video
		</p>
		<div style="display:none;"  id="video-block">
			<a {if $arrPrj.flg_status>=$arrStatus.pending} style="display:none;"{/if}
				href="{url name='site1_video_manager' action='multibox'}?multiselect=1" class="mb" rel="">Import from Video Manager</a>
			<br />
			<br />
		...
		<p>
			<input type="checkbox" id="article-content"{if  $arrPrj.flg_status>=$arrStatus.pending} disabled='1'{/if} /> Articles
		</p>
		{/if}
		<div style="display:none;" id="article-block">
		...
		<p>
			{if !empty($arrPrj.id)}
			<small>Note: if you would like to edit any of the content with 'Approved' status, you would need to delete it from the project, then edit it, and then re-add it to your syndication project. You can edit content with 'Rejected' or 'Draft' status without needing to delete and re-add it.</small><br/>
			{/if}
			<small>IMPORTANT: if you have submitted a project for review, and it got 'Approved', you are NOT able to edit any of the project content as it has already been submitted for posting.</small>
		</p>
	</fieldset>
	...
	<fieldset>
		...
		<p>
			<label>Project name <em>*</em></label><input type="text" class="required text-input medium-input" name="arrPrj[title]" value="{$arrPrj.title}" />
		</p>
		<p>
			<label>Select category <em>*</em></label>

			<select class="required validate-custom-required emptyValue:'0' article_source medium-input" {if $arrPrj.flg_status>=$arrStatus.approved}disabled='1'{/if} id="select-category"><option value=""> - select -</select>

		</p>
		<div id="category-block" style="display:none;">
			<input type="checkbox" {if $arrPrj.flg_status>=$arrStatus.approved}disabled='1'{/if} id="select-all-category"> Select all<br/>
	...
	<fieldset>
		...
		<p>
			...
			<input type="checkbox"{if $arrPrj.flg_status>=$arrStatus.approved} disabled=""{/if}{if $arrPrj.flg_backlinks != 0} checked=""{/if} id="open-backlinking"  /> Backlinking
		</p>
		...
		<p>
			...
			<input  type="radio" id="default-backlinking" {if $arrPrj.flg_status>=$arrStatus.approved}disabled='1'{/if} value="1" {if $arrPrj.flg_backlinks == 1} checked='1' {/if} name="arrPrj[flg_backlinks]" class="radio-hands" /> My sites from project category
		</p>
		<p>
			<input type="radio" {if $arrPrj.flg_status>=$arrStatus.approved}disabled='1'{/if} name="arrPrj[flg_backlinks]" {if $arrPrj.flg_backlinks == 2} checked='1' {/if} value="2" class="radio-hands" /> Manual select
		</p>
		<div style="display:{if $arrPrj.flg_backlinks == 2}block{else}none{/if};" id="hands">
		<p>
			<label>&nbsp;</label>
{if Core_Acs::haveAccess( 'email test group','CNM1.0' )}
			<a {if $arrPrj.flg_status>=$arrStatus.approved}style="display:none;"{/if} href="{url name="site1_ncsb" action="multiboxlist"}" class="mb" rel="">NCSB</a>&nbsp;&nbsp;&nbsp;
			<a {if $arrPrj.flg_status>=$arrStatus.approved}style="display:none;"{/if} href="{url name="site1_nvsb" action="multiboxlist"}" class="mb" rel="">NVSB</a>&nbsp;&nbsp;&nbsp;
{/if}
			<a {if $arrPrj.flg_status>=$arrStatus.approved}style="display:none;"{/if} href="{url name="site1_blogfusion" action="multiboxlist"}?noversion=1" class="mb" rel="">Blog fusion</a>&nbsp;&nbsp;&nbsp;
		...
		<p>
			<label>Additional 3rd party sites</label><textarea{if $arrPrj.flg_status>=$arrStatus.approved} disabled=""{/if} name="arrPrj[backlinks]" id="place-for-link" style="height:150px;">{$arrPrj.backlinks}</textarea><br/>
			<small>Notice: Separate urls by comma, space or newline</small><br/>
			<small>Link format example: http://www.mydomain.com/ </small>
		</p>
		</div>
		<p>
			<input type="submit" value="Save" class="save button" />
			<input type="submit" name="arrPrj[for_review]" value="Submit for review" class="button save"{if $arrPrj.flg_status>=$arrStatus.approved||$arrPoints.balance<=0} style="display:none;"{/if} />
		</p>
	</fieldset>
		...
		for( var i=1; i<6; i++ ){
			if( $('site_place_' + i ) && $chk(arrData) ){
				...
				arrData.each( function( value ) {
					if( value!=null && value.title!=null && i == value.flg_type ) {
