...
<fieldset>
		...
		<div>
			<label>Campaigns: <em>*</em></label>
			{foreach from=$arrSplit.compains item='compain' key='id'}
				<div class="checkbox checkbox-primary">
					<input type="checkbox" class="com_chose" value="{$compain.id}" name="arrSplit[arrCom][]"{foreach from=$arrSplit.arrCom item='checked_com_id'}{if $checked_com_id == $compain.id} checked="checked"{/if}{/foreach} />
					<label>{$compain.title}</label>	
				</div>
			{foreachelse}
			<label>Please crate two or more campaignes</label>
			{/foreach}
			<div style="display:none" id="com_chose_error" class="validation-advice input-notification error png_bg">Error: Please check two or more campaignes.</div>
		</div>
		<div class="form-group">
		...
		<div class="flg_duration_type"{if $arrSplit.flg_duration !='1' && $arrSplit.flg_duration !='2'} style="display:none;"{/if}>
			<label><span class="flg_duration_1"{if $arrSplit.flg_duration !='1'} style="display:none;"{/if}>Duration: </span>
				<span class="flg_duration_2"{if $arrSplit.flg_duration !='2'} style="display:none;"{/if}>No. of Hits: </span>
				<em>*</em></label>
			<input type="text" id="duration" class="text-input small-input validate-natural-number" value="{$arrSplit.duration}" name="arrSplit[duration]"/>&nbsp;
				...
				<span class="flg_duration_2"{if $arrSplit.flg_duration !='2'} style="display:none;"{/if}>Each Campaign</span>
		</div>
</fieldset>
<br />
	<div>
