...
It's recommended you customize your <a href="{url name='site1_funnels' action='settings'}">Affiliate and Autoresponder Settings</a> to get started and <a href="{url name='site1_funnels' action='create'}">create</a> your first Funnel.
</div>
{else}
<style type="text/css">
		...
		<div class="col-md-3">
			<div class="widget-bg-color-icon card-box fadeInDown animated">
				...
				<div class="text-right">
					<h3 class="text-dark"><b class="counter">{Core_Users::$info['statistic']['lpb_campaigns_count']}</b></h3>
...
{if $arrList}
			...
			<div class="card-box">
				...
				<div class="text-center">
					<ul class="list-inline chart-detail-list">
						<li>
							<h5><i class="fa fa-circle m-r-5" style="color: #5fbeaa;"></i>Clicks</h5>
						</li>
						<li>
	...
	<div class="card-box">
		...
		<table class="table table-striped" id="visited">
			...
			<tbody>
			{foreach $arrList as $v}{if $v.visitors>0}
			<tr class="{if ($v@iteration-1) is div by 2}matros{/if}{if ($v@iteration)>$rows_per_page} hidden{/if}">
				<td>{$v.visitors}</td>
				<td>{$v.clicks}</td>
				<td>{sprintf("%01.2f", $v.rate)}</td>
				<td><a href="{$v.url}" target="_blank">{$v.url}</a></td>
				<td><a href="{url name='site1_funnels' action='dashboard'}?id={$v.id}{if !empty($sortParam)}&{$sortParam}{/if}">Report</a></td>
			</tr>
			{/if}{/foreach}
			</tbody>
		</table>	
		...
		<table class="table table-striped" id="location">
			<thead>
				<tr>
					...
					<th>Conversions{include file="../../ord_frontend2.tpl" field='click'}</th>
	...
	<div class="row">
			<div class="col-lg-2 col-md-4 col-sm-5 col-xs-6">
				<select name="arrFilter[utm_source]" class="btn-group selectpicker show-tick">
					...
					<option {if $smarty.get.arrFilter.utm_source==$sourceFilter} selected="selected" {/if} value="{$sourceFilter}">{$sourceFilter}</option>{/foreach}
				</select>
			</div>
			<div class="col-lg-2 col-md-4 col-sm-5 col-xs-6">
				<select name="arrFilter[utm_medium]" class="btn-group selectpicker show-tick">
					...
					<option {if $smarty.get.arrFilter.utm_medium==$sourceFilter} selected="selected" {/if} value="{$sourceFilter}">{$sourceFilter}</option>{/foreach}
				</select>
			</div>
			<div class="col-lg-2 col-md-4 col-sm-5 col-xs-6">
				<select name="arrFilter[utm_campaign]" class="btn-group selectpicker show-tick">
					...
					<option {if $smarty.get.arrFilter.utm_campaign==$sourceFilter} selected="selected" {/if} value="{$sourceFilter}">{$sourceFilter}</option>{/foreach}
				</select>
			</div>
			<div class="col-md-3 col-sm-3 col-xs-12">
				<button type="submit" class="btn btn-success waves-effect waves-light">Filter</button>
